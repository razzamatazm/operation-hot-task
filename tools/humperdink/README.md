...
its name and the page's URL — to your clipboard as JSON. Over in Hot Task, the
create form's **Import from Humperdink** button takes that paste and fills
Folder Name and the Humperdink Link.
...
2. In Hot Task, open **New Task**, click into **Paste from Humperdink**, paste,
   and press **Import from Humperdink**. Folder Name and the Humperdink Link
   fill in and the button reads `Imported`.
...
Nothing here is scraped by CSS selector. The loan name comes off the page title
(`<LoanName> - Details`) and the link off the address bar, which is why this
version survives markup changes. That stops being true as the scrape grows —
[#196](https://github.com/razzamatazm/operation-hot-task/issues/196) and
[#197](https://github.com/razzamatazm/operation-hot-task/issues/197) read loan
terms, contacts and properties by element id, and those are the parts that will
need maintaining.

For that work, use a saved copy of a real loan details page as the selector
reference. The page is ~1.3 MB of HTML plus a few hundred asset files and is
customer data, so it is not committed here — save your own from the browser
(**Save page as → Webpage, Complete**) and keep it outside the repo.
