...
/* When the person currently holding a task took it on, read out of the task's
   ...
   Two vocabularies of history action put a holder on a task, and two take one
   off:
   ...
   unclaims would keep quoting a holder who has already stepped off.
...
const HOLDER_TAKEN = new Set(["TASK_CLAIMED", "TASK_ASSIGNED"]);
const HOLDER_CLEARED = new Set(["TASK_UNCLAIMED", "TASK_RELEASED"]);

const at = (event: Pick<TaskHistoryEvent, "at">): number => new Date(event.at).getTime();

/* The instant the current holder took the task, or `undefined` when nobody
   holds it — or when history is empty, truncated, or predates the row.
   ...
   would credit the current holder with a previous person's shift.
   ...
   Says nothing about *who* holds the task — the caller has that from
   ...
   the clearing event. */
export const currentHolderSince = (history: readonly Pick<TaskHistoryEvent, "action" | "at">[]): string | undefined => {
  let takenAt: string | undefined;
  for (const event of [...history].sort((a, b) => at(a) - at(b))) {
    if (HOLDER_TAKEN.has(event.action)) takenAt = event.at;
    else if (HOLDER_CLEARED.has(event.action)) takenAt = undefined;
  }
  return takenAt;
};
