...
const TaskCard = memo(({
  ...
  const secondaryActionsBlock = showActions && cancelStage === "idle" && (
    <>
      ...
      {/* #125: the reverse move out of review. NEEDS_REVIEW's forward move
          (Complete) rides the collapsed row (#118); handing the task back to
          the assignee is the rarer, backwards step, so it stays in the menu —
          same shape and placement as `Undo Merge Done` below. Gated by the
          shared canMoveNeedsReview (creator / assignee / admin) so the menu
  ...
  /* Actions menu: hamburger next to the row's primary action (see the
     ...
     Done, Undo Review). Cancel's confirm/done UI renders inside the same panel so it
