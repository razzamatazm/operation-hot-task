...
- Due date is tracked backend-only and is not shown as a dedicated field in
  the main tab UI. Exception: the DM task cards do surface a `Due` line —
  the conversation card a claim sends both parties, and the handoff's details
  card — see [notifications-bot.md](notifications-bot.md#routing).
