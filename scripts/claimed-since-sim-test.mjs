...
/* Issue #166 — the hamburger's timestamp block gains a line saying when the
   ...
   out of the task's history. `currentHolderSince` (packages/shared/src/history.ts)
...
import { currentHolderSince } from "../packages/shared/src/history.ts";
...
test("a claimed task reports the claim", () => {
  assert.equal(
    currentHolderSince([CREATED, event("TASK_CLAIMED", "2026-03-01T09:30:00.000Z")]),
...
test("a handoff reports when the recipient received it, not the previous claim", () => {
  ...
  assert.equal(currentHolderSince(history), "2026-03-01T14:00:00.000Z");
});
...
test("a task born assigned reports its creation-time assignment", () => {
  ...
  assert.equal(currentHolderSince(history), "2026-03-01T09:00:00.000Z");
});
...
test("a task claimed, returned to the pool and re-claimed reports the SECOND claim", () => {
  ...
  assert.equal(currentHolderSince(history), "2026-03-02T08:15:00.000Z");
});
...
test("a release in place clears the holder just as an unclaim does", () => {
  ...
  assert.equal(currentHolderSince(history), undefined, "released for any checker — nobody holds it");
});
...
test("noise between the doors does not disturb the answer", () => {
  ...
  assert.equal(currentHolderSince(history), "2026-03-01T09:30:00.000Z");
});
...
test("chronology is established here, not trusted from the caller", () => {
  ...
  assert.equal(currentHolderSince(shuffled), "2026-03-02T08:15:00.000Z");
});

/* ── Nothing to say ────────────────────────────────────── */

test("a task nobody has taken reports nothing", () => {
  assert.equal(currentHolderSince([CREATED]), undefined);
});
...
test("a task returned to the pool reports nothing", () => {
  ...
  assert.equal(currentHolderSince(history), undefined);
});

test("empty history reports nothing rather than throwing", () => {
  assert.equal(currentHolderSince([]), undefined, "history dropped by retention, or a task that predates the row");
});
